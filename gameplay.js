document.addEventListener('DOMContentLoaded', function () {
    ...
    let hardMode = false; //game begins in normal mode
    let score = 0;
    ...
    const modeToggleImgSrc = 'img/white-normal-mode-toggle-Photoroom.png';
    modeToggleScale = 2.1;
    const modeToggleWidth = 97 * modeToggleScale;  // Change as needed
    const modeToggleHeight = 60 * modeToggleScale; // Change as needed
    const toggleX = canvas.width / 100;
    const toggleY = canvas.height / 100;
        ...
        if (lvl > 0 && !loadingScreen) {
            ...
            function drawStartScreenLogo() {
                ...
                logoImg.onload = function () {
                    ...
                    const logoY = canvas.height / 2 + 200;
                    const padding = 24;
            ...
            // Add a textbox, start button, toggle image, and label to the screen
            ...
            if (!existingDiv) {
                ...
                // Toggle image (as a DOM element)
                const toggleImg = document.createElement('img');
                toggleImg.src = modeToggleImgSrc;
                toggleImg.alt = 'Mode Toggle';
                toggleImg.id = 'modeToggleImg';

                // Toggle label (optional)
                const toggleLabel = document.createElement('div');
                toggleLabel.innerText = 'Casual Mode';
                toggleLabel.id = 'modeToggleLabel';

                // Append toggle and label after the start button
                container.appendChild(toggleImg);
                container.appendChild(toggleLabel);

                document.body.appendChild(container);

                // Add event listener for toggle image
                toggleImg.addEventListener('click', function () {
                    if (!hardMode) {
                        toggleImg.src = 'img/black-hard-mode-toggle-Photoroom.png';
                        hardMode = true;
                        toggleLabel.innerText = 'Extreme Mode'
                        startBtn.style.color ='rgb(196, 79, 66)';
                    } else {
                        toggleImg.src = 'img/white-normal-mode-toggle-Photoroom.png';
                        hardMode = false;
                        toggleLabel.innerText = 'Casual Mode';
                        startBtn.style.color ='rgb(255, 201, 7)';
                    }
                });
            }
    ...
    function lvlStats() {
        if (!hardMode) {
            ...
            console.log("casual level stats adjusted for day", lvl);
        } else {
            // Only update player properties if player is defined
            if (lvl == 1) {
                village_drain_speed = 3.2;
                animalSpawnAmount = 4;
                villageSpawnAmount = 4;
                animal_drain_speed = 10;
            } else if (lvl == 2) {
                drainSpeed = 1.8; //Decreases player bucket size;
                animalSpawnAmount = 5;
                if (typeof player !== "undefined") player.speed = 5.5; //Player Buff (player speed increased)
            } else if (lvl == 3) {
                villageSpawnAmount = 5; //Add a village
                village_drain_speed = 3.4; //Increase speed villages will drain
            } else if (lvl == 4) {
                if (typeof player !== "undefined") player.speed = 6.5; //Player Buff (player speed increased)
                village_drain_speed = 3.3; //Increase speed villages will drain
                animalSpawnAmount = 6;
                ...
                drainSpeed = 2; //Player Nerf (increases bucket size by decreasing drain speed)
                animalSpawnAmount = 8;
            } else if (lvl == 6) {
                if (typeof player !== "undefined") player.speed = 7; //Player Buff (player speed increased)
                village_drain_speed = 3.6; //Decrease speed villages will drain back to initial
                villageSpawnAmount = 8
            } else if (lvl == 7) {
                villageSpawnAmount = 9; //Add a village
                if (typeof player !== "undefined") player.speed = 8; //Player Buff (player speed increased)
                animalSpawnAmount = 9
            } else if (lvl > 7) {
                village_drain_speed += 0.1; //Increase speed villages will drain each round
            ...
            console.log("hard level stats adjusted for day", lvl);
        }
    ...
    function updateDayCounter() {
        ...
        if (!hardMode) {
            dayDiv.style.color = 'rgba(255, 255, 255, 0.85)';

        } else {
            dayDiv.style.color = 'rgba(196, 79, 66, 0.85)';
        }
    }
    ...
    function updateScoreCounter() {
        ...
        if (!hardMode) {
            scoreDiv.style.color = 'rgba(255, 255, 255, 0.85)';

        } else {
            scoreDiv.style.color = 'rgba(196, 79, 66, 0.85)';
        }
    }
    ...
    function updateTimerDisplay(running) {
        ...
        if (!timerDiv) {
            ...
            if (!hardMode) timerDiv.style.color = 'rgba(255, 255, 255, 0.85)';
            else timerDiv.style.color = 'rgba(196, 79, 66, 0.85)'
